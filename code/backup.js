...
(function ($, Swal10, PLUGIN_ID) {
  ...
  kintone.events.on(['app.record.edit.show', 'app.record.create.show'], async function (event) {
    try {
      ...
      $.each(TRANSLATE_FIELDS, function(index, item){
        if (item.space_element) {
          if (!spaceElementsMap[item.space_element]) {
            spaceElementsMap[item.space_element] = item;
            fieldButton[item.space_element] = [];
            for(let i = 0; i<item.target_fields.length; i++){
              if (item.target_fields[i].field){
                fieldButton[item.space_element][i] = true;
              }else{
                fieldButton[item.space_element][i] = false;
              }
            }
          } else {
            const existingItem = spaceElementsMap[item.space_element];
            for (let i = 0; i < item.target_fields.length; i++) {
      ...
      // unique list as an array
      let uniqueSpaceElements = [...new Set(TRANSLATE_FIELDS.map(item => item.space_element))].filter(Boolean);

      //create button translate to space element
      $.each(uniqueSpaceElements, function (index, space) {//comment use spaceElementMap
        ...
        $.each(LANGUAGE_LIST, function (i, lang) {//comment fix loop
          ...
          if (fieldButton[space][i] === false && lang.iso.toUpperCase() === ISO_DEFAULT.toUpperCase()) {
